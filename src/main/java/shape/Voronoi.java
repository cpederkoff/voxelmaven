...
public class Voronoi extends Shape {
     ...
     * @param targetShape
     */
    public Voronoi(double thickness, int dimension, Shape targetShape,
            Point3d[] points, double keepAway) {
    ...
    private double getPointDiff(final Point3d point, Point3d[] points) {
        ...
        double otherdist = points[dimension].distance(point)
                - points[0].distance(point);
        return otherdist;
    }
    ...
    public long fastInShape(Point3d point) {
        ...
        for (Point3d p : points) {
            ...
            if (p.y > point.y - cutoff && p.y < point.y + cutoff
                    && p.z > point.z - cutoff && p.z < point.z + cutoff
        ...
        for (int x = 0; x < 63; x++) {
            if (arr.length > dimension
                    && getPointDiff(new Point3d(point.x + x, point.y, point.z),
                            arr) < thickness) {
                ret |= 0x1;
            }
            ret = ret << 1;
        ...
        if (arr.length > dimension
                && getPointDiff(new Point3d(point.x + 63, point.y, point.z),
                        arr) < thickness) {
            ret |= 0x1;
        }
        return ret;
