...
public class ShapeTest extends TestCase {
    ...
    public void testVoronoi() {
        ...
        checkFastInShape(new Voronoi(5, 2, cube,
                voronoiPoints.generatePoints(), voronoiPoints.getKeepAway()));
    }
                ...
                for (int x = (int) bounds[0]; x < (int) bounds[1]; x += 64) {
                    ...
                    for (int i = 0; i < 64; i++) {
                        if (x + i < bounds[1]) {
                            ...
                            assertEquals(
                                    String.format(
                                            "Didn't match at coordinate x:%d, y:%d, z:%d",
                                            x + i, y, z), fastIsIn,
                                    shape.inShape(new Point3d(x + i, y, z)));
                        }
                        vals = vals << 1;
                    }
